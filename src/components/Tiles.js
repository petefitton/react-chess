    ...
    for (let char=0; char<piecePlacement.length; char++) {
      if (/^[1-8]+$/.test(piecePlacement[char])) {
        ...
        for (let j=0; j<piecePlacement[char]; j++) {
          (function(i) {
            setTiles(oldTiles => [...oldTiles, <Tile key={tileLocations[i]} id={tileLocations[i]} handleTileClick={handleTileClick} isWhite={Math.floor(i / 8) % 2 === 0 ? i % 2 === 0 : i % 2 !== 0} />]);
          })(i);
          i++;
        }
      }
        ...
        // there is a piece
        (function(i) {
          setTiles(oldTiles => [...oldTiles, <Tile children={<CreatePiece pieceType={piecePlacement[char]} key={i} />} key={tileLocations[i]} id={tileLocations[i]} handleTileClick={handleTileClick} isWhite={Math.floor(i / 8) % 2 === 0 ? i % 2 === 0 : i % 2 !== 0} />]);
        })(i);
        i++;
      }
    }
